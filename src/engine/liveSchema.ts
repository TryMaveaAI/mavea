  ...
  const criteria: CmpCriterion[] = asArr(p.criteria)
    .map((c): CmpCriterion | null => {
      ...
      const cells: CmpCell[] = asArr(co.cells).map((cell): CmpCell => {
        // `v` is the schema's key, but it is also the tersest in the app — a model reaching for
        // {value}/{text}, or a bare "High", used to coerce to v:'' and the card rendered its
        // full grid with every cell blank (labels and headers intact, nothing in between).
    ...
    // A row whose every cell is still blank has nothing to show between its label and the

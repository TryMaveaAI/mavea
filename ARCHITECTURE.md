...
`charts1 charts2 stats tables flows docs ai briefs media layout status diagrams learn code everyday
...
(typically 4–8 of 24), prefetched while the answer streams so every card still mounts together.
...
### One layout contract for every block and surface

The same block renders in five different boxes — a card on the board, the Lens stage, the Study
slot, the world's parts rail and a gallery tile — and every surface is read from a 320px phone to a
3440px ultrawide, at whatever zoom the reader chose. So layout answers to tokens, not to numbers
typed into a component: `src/styles/tokens-base.css` holds the breakpoint ladder (range syntax,
documented there and enforced from `scripts/breakpoints.mjs`), the fluid type ramp (`--fs-2xs` …
`--fs-hero`, Utopia's method — each step is a straight line between its size at 320px and at
1920px, regenerated by `scripts/utopia-scale.mjs`, composed with the reader's text-size knob and the
viewport's upward scale), the container-relative spacing (`--sp-fluid-*` in `cqi`, so a block's
rhythm tightens with its card rather than the window), and the query containers (`.card`,
`.card-grid`, the rails, the bar, the dock). stylelint (`stylelint.config.js`) refuses a fixed px
width, a px font size, a bare `vh`, vertical writing modes, an unexplained `nowrap` and any media
query off the ladder; an exception states its reason where it lives. The geometry suite
(`pnpm audit:responsive`, `scripts/surface-audit.mts` over the sweep table in
`scripts/surface-sweep.mjs`) then measures what the rules cannot see — horizontal scroll, clipping,
collapsed columns, overlapping text, the thumb floor — on every route in `src/routeTable.ts`, a
table a test holds the sweep against so a new route cannot go unmeasured.
...
```
 ...
 └───────────────────────────┘      │    clamp #s, cap count, │                              donut   → Donut …
   ...
   (Kokoro / browser TTS)          │ what it cannot fix is   │
                                    │   left on the answer    │
                                    └────────────────────────┘
```
...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Conversation shell** — answer hero, dock composer, session rail, 6×2 theme templates                                                                                                                                                               | `voice/`, `templates.ts`                                                              | presentation only; templates are full token-contract rebinds under `data-template`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| **The revision grammar** — a turn REPLACES the canvas, ADDS to it, or EDITS cards already on it, and the board says which                                                                                                                            | `lifecycle.ts`, `settleTurn.ts`, `revise/classifyRevision.ts`                         | the mode comes from the model's `continuity` hint, a deterministic topic decision, and the reader's own words (`classifyRevision` — an instruction to change something on screen is the only route to `refine` that does not need a frontier model to volunteer it). `mergeForMode` returns a `MergeDelta` saying which cards it edited and which it added, read off the merged array so it cannot disagree with it; it rides the frame as an optional field, and every consumer gates it on `answerSignature` because block ids are positional and reused across turns                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **The board · the Lens · The Study · Focus** — the resting grid; clicking a card opens it on its own stage with Mavéa's notes beside it; plus two takeovers of one answer: a lamplit desk you ask for, and one card at a time (which Present drives) | `../canvas/study/`, `../canvas/focus/`, `content/studyVoices.ts`, `study/annotate.ts` | one persisted value (`useFocusMode`: cache + `localStorage` + `CustomEvent`, and `board` is the only thing ever written); every view re-casts the SAME `Block[]` through the same renderer, so no view has a second render path. The Lens is not a view at all — it is the zoom sheet repurposed: clicking a card opens it centred over a blurred board, with magnification as one control on that stage rather than a second pill beside it. `TopicCanvas` owns the stage and reports it via `onLens`, so the surface only has to write that one card's notes; those are the desk's own `studyVoices`, which derives every slot it has no written note for, so the Lens costs no model call and the written upgrade stays behind the desk latch. The desk's four margin notes per object are derived from the block and the turn's real sources (`content/studyVoices.ts`); Mavéa's own written notes are a separate streamed side-channel call (`study/annotate.ts`) with its own prompt, its own deadline and a cache keyed on the blocks' content digest — bought once per answer, never on the turn path |
...
| `mavea-view-mode`           | App / Live    | `'board'` (string) — the only view ever written; the rest are takeovers                                                                  | No                                            | Always                                                          |
| `mavea-desk-first`          | Live          | `'yes'` once the Study's desk has been opened (arms its prefetched notes)                                                                | No                                            | Always                                                          |
| `mavea-gemini-no-minimal`   | Live          | `string[]` of Gemini model ids that refused `thinkingLevel: MINIMAL`, so the refusal costs one request per model ever, not per page load | No                                            | Always                                                          |
...
The **view mode** (`canvas/focus/useFocusMode.ts`) is shared across the Demo and Live (one key).
`board` — the full grid — is where the canvas rests, and it is the ONLY value ever written: a
reader is never asked to name how they want to read before they have read anything. Every other
view is a takeover of ONE answer, entered by a control and left by one — the Study's desk, Focus's
single card (which Present drives), the spatial canvas and the causal world — so none is written to
the key and none can clobber the standing choice. Values saved by older builds (`room`, `study`,
`focus`, `everything`) are read as `board`; a reader who used the desk is recognised instead by
`live/study/deskHabit.ts`, which LATCHES, because the first write to the view key replaces the
value it seeds from. That flag — not the view — is what arms the Study's prefetched margin notes,
the generous pen and the note gutter.
...
reverse proxy, model/search providers, local speech services, and media hosts remain separate trust
boundaries with their own logging and retention policies.
...
In development, `vite.config.ts` proxies `/llm/*` to the model providers, so API keys set by the
user travel browser → local Vite dev server → provider.
...
| To add a…        | Touch                                                                                                                                     |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| **Demo persona** | a cast entry in `demo/cast.ts` + a script in `demo/scripts.ts`, then bake it: `ONLY=<id> node --import tsx scripts/build-demo-corpus.mts` |

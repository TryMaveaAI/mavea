...
| **The living world** — any "why" answer as its causal web: walk it, weigh it, read it in time                         | `world/`, `trust/`, `../canvas/spatial/morph/`                             | a `world` block on the ordinary canvas opens a full-screen spatial surface; one `WorldSpec` renders as four representations from pure `LayoutFn`s (what led to what · measured contribution · when · what each measured), a view is only OFFERED where it can be filled, and every figure resolves through the trust registry or does not render. A what-if re-runs the cascade locally (no model call) and re-weights the world IN PLACE, worded by a digit-free vocabulary. "Walk me through it" narrates beats composed from the spec itself — zero tokens |
...
## The living world: an answer you can walk
...
**One spec, four representations.** `canvas/spatial/morph/` is a general spatial renderer that

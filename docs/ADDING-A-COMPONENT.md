...
> `src/canvas/blocks/catalog/catalog.data.ts`. That is the single source of truth the selector,
> prompt, and coercer all read. `registry-types.ts` defines only the _render_ contract
> (`BlockRegistry` + `BlockCommon`); it is not a second metadata system.
...
3. Verify: `pnpm typecheck && pnpm test && pnpm knip`.
